...
func Append(ctx context.Context, basen ipld.Node, db *h.DagBuilderHelper) (out ipld.Node, errOut error) {
	...
	for i := n; !db.Done(); i++ {
		for j := 0; j < layerRepeat && !db.Done(); j++ {
			next := db.NewFSNodeOverDag(ft.TFile)
			childNode, childFileSize, err := fillTrickleRecFSNode(db, next, i)
			if err != nil {
				return nil, err
			}
...
func appendFillLastChild(ctx context.Context, fsn *h.FSNodeOverDag, depth int, layerFill int, db *h.DagBuilderHelper) error {
	...
	nodeFileSize := nchild.FileSize()
	filledNode, err := nchild.Commit()
	if err != nil {
		return err
	}

	err = fsn.AddChild(filledNode, nodeFileSize, db)
	if err != nil {
		return err
	}
...
func appendRec(ctx context.Context, fsn *h.FSNodeOverDag, db *h.DagBuilderHelper, depth int) (*h.FSNodeOverDag, error) {
	if depth == 0 || db.Done() {
		return fsn, nil
	}
